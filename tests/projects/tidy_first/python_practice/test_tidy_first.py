...
from projects.python_practice.tidy_first import (
    ...
    process_user_untidy,
    process_user_tidy,
    validate_email_untidy,
    validate_email_tidy,
    calculate_salary_untidy,
    calculate_salary_tidy,
    calculate_shipping_untidy,
    calculate_shipping_tidy,
    apply_standard_discount_untidy,
    apply_discount_tidy,
    should_grant_access_untidy,
    should_grant_access_tidy,
    process_order_untidy,
    process_order_tidy,
    calc_cost_untidy,
    calculate_cost_tidy,
    validate_user_profile_untidy,
    validate_user_profile_tidy,
)


@pytest.mark.parametrize(
    "customer_type, amount, member, expected",
    [
...
def test_p6_discount_symmetry():
    price = 100
    assert apply_standard_discount_untidy(price) == apply_discount_tidy(
        price, "standard"
    )


@pytest.mark.parametrize(
    "role, active, balance, expected",
    [

...
The app already uses `layout="centered"` and custom CSS; a dedicated **mobile layout** pass is orthogonal to **reachability**.

## Recommendation

**Primary path (phase 1): Cloudflare Tunnel + Access** to the Streamlit process on your Mac (or a always-on home machine).
...
1. **`.streamlit/config.toml`** — `headless = true`, usage stats off (typical for tunnel/server).
2. **`just grocery-ui-remote`** — binds `0.0.0.0:8501` so a local tunnel agent can forward to Streamlit (same as many Cloudflare Tunnel / ngrok tutorials).
...
These are tracked as separate GitHub backlog items (label `grocery-wizard`):

| Phase | Topic |
| --- | --- |
| 1b | Documented/automated Cloudflare Tunnel + Access setup for this repo |
| 2 | Always-on Streamlit deploy (container + platform secrets) |
| 3 | Access control in-app or at edge if not using Cloudflare Access |
| 4 | Mobile UX: Add to Home Screen hints, optional PWA meta, touch layout pass |

Issue numbers are listed in the PR that closes #129 after those issues are created.
